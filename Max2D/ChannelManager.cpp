...
namespace hoa
{
	...
	{
        m_fisheyeDestAzimuth = Math<double>::wrap_twopi(azimuth);
		m_fisheyeStep = 0;
	}
	
	void ChannelManager::setFisheyeStepWithDelta(const int index, double delta)
	{
		m_fisheyeStep = Math<double>::clip(m_fisheyeStep + delta, 0, 1);
	...
	{
		double start = Math<double>::wrap_twopi(startRad);
		double end   = Math<double>::wrap_twopi(endRad);
		
		// anti-clockwise
		
		if (Math<double>::wrap_twopi(end - start) <= HOA_PI)
		{
			if (end - start >= 0)
				return Math<double>::wrap_twopi(start + step*(end - start));
			else
				return Math<double>::wrap_twopi(start + step * ((end + HOA_2PI) - start));
		}
		...
		else
		{
			if (end - start <= 0)
				...
				return Math<double>::wrap_twopi( start - step*( (start + HOA_2PI) - end) );
		}
	}
	...
	{
		double angle = Math<double>::wrap_twopi(azimuth);
