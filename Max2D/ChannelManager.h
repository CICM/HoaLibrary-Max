...
namespace hoa
{
	...
	private:
		...
		public:
			...
			double getWidening() const noexcept
            {
                return m_wideningValue;
            }
            
			double getFisheyeStartAzimuth() const noexcept
            {
                return m_fisheyeStartAzimuth;
            }
            
            double getAbscissa() const noexcept
            {
                return Math<double>::abscissa(1.0f, m_azimuth);
            }
            
			double getOrdinate() const noexcept
            {
                return Math<double>::ordinate(1.0f, m_azimuth);
            }
            
			double getAzimuth() const noexcept
            {
                return m_azimuth;
            }
			
			void setAzimuth(const double angle)
            {
                m_azimuth = Math<double>::wrap_twopi(angle);
            }
            
			void rotateAzimuth(const double deltaAzimuth)
            {
                m_azimuth = Math<double>::wrap_twopi(m_azimuth + deltaAzimuth);
            }
			
            void setWideningValue(const double widerValue)
            {
                m_wideningValue = Math<double>::clip(widerValue, 0, 1);
            }
            
			void setFisheyeStartAzimuth()
            {
                m_fisheyeStartAzimuth = m_azimuth;
            }
            
			void setFisheyeStartAzimuth(const double radian)
            {
                m_fisheyeStartAzimuth = Math<double>::wrap_twopi(radian);
            }
			
            void setFisheyeEndAngle()
            {
                m_fisheyeEndAzimuth = m_azimuth;
            }
            
			void setFisheyeEndAngle(const double radian)
            {
                m_fisheyeEndAzimuth = Math<double>::wrap_twopi(radian);
            }
			
            void setSelected(const int state)
            {
                m_isSelected = (state == -1) ? !m_isSelected : (state != 0);
            }
		};
        ...
        inline double radianClosestDistance(double angle1, double angle2)
        {
            ...
            angle1 = Math<double>::wrap_twopi(angle1);
            angle2 = Math<double>::wrap_twopi(angle2);
