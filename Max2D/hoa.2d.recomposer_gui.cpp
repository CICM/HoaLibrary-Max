...
void hoa_2d_recomposer_gui_preset(t_hoa_2d_recomposer_gui *x)
{
    ...
    for(int i = 0; i < x->f_number_of_channels; i++)
    {
        ...
        binbuf_vinsert(z, gensym("osslf")->s_name, x, object_classname(x), hoa_sym_wide, i, (float)x->f_channels->getWidening(i));
    }
}
...
t_max_err hoa_2d_recomposer_gui_setvalueof(t_hoa_2d_recomposer_gui *x, long ac, t_atom *av)
{
    if (ac && av)
	{
        ...
        {
            x->f_channels->setAzimuth(index, Math<double>::azimuth(atom_getfloat(av+i), atom_getfloat(av+i+1)));
    ...
    if (ac && av)
    {
        if (*ac && *av)
        {
            ...
            for (i = index = 0; index < x->f_number_of_channels; index++, i+=3)
            {
                ...
                atom_setfloat(*av+i+2, x->f_channels->getWidening(index) );
            }
		}
        else
        {
...
void hoa_2d_recomposer_gui_output(t_hoa_2d_recomposer_gui *x)
{
    ...
    for (int i=0; i<channels; i++)
        atom_setfloat(av_left+i, x->f_channels->getWidening(i));
...
void computeRepresentation(t_hoa_2d_recomposer_gui *x, int index)
{
	...
	x->f_wider->setWidening(x->f_channels->getWidening(index));
...
void hoa_2d_recomposer_gui_mousedrag(t_hoa_2d_recomposer_gui *x, t_object *patcherview, t_pt pt, long modifiers)
{
    ...
    else if (x->f_last_mouseDownOverChannel != -1)
    {
        if (modifiers == 18) // shift => set wider
        {
			...
			{
				if (x->f_channels->isSelected(i))
					x->f_channels->setWideningValue( i, x->f_channels->getWidening(i) + radiusDelta);
			}
        }
        else // => simply rotate
        {
...
int isPointOverAChannel(t_hoa_2d_recomposer_gui *x, t_pt *pt)
{
    ...
    {
		if ( hoa::isInsideRad(azimuth, x->f_channels->getAzimuth(i) - chanSize, x->f_channels->getAzimuth(i) + chanSize) &&
...
bool isChannelInsideRect(t_hoa_2d_recomposer_gui *x, int micIndex, t_rect rectSelection)
{
    double w = x->rect.width;
    t_pt micPoint;
    micPoint.x = Math<double>::abscissa(x->f_channel_radius, x->f_channels->getAzimuth(micIndex));
    micPoint.y = Math<double>::ordinate(x->f_channel_radius, x->f_channels->getAzimuth(micIndex));
