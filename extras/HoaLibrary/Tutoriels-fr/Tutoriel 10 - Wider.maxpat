...
, 			{
				"box" : 				{
					...
					"style" : "",
					"text" : "Cette opération consiste à multiplier chaque harmonique par un gain g qui dépend de l'ordre de décomposition N, du degré de l'harmonique l et de la valeur d'élargissement x :\nif (l == 0)\n    g(x) = b(x) * N + 1\nelse\n    g(x) = (b(x) * N - 1) + 1) * (cos(min(0, max(a(x) * l, π))) + 1) * 0.5\nwith\n    a(x) = (1 - x) * π\nand \n    b(x) = (sin(a(x) - π / 2) + 1) * 0.5",
					"textcolor" : [ 0.317518, 0.317518, 0.317518, 1.0 ]
				}
...
, 			{
				"box" : 				{
					...
					"style" : "",
					"text" : "Valeur d'élargissement entre 0 (omnidirectionel) et 1 (le plus directionel).",
					"textcolor" : [ 0.317518, 0.317518, 0.317518, 1.0 ]
				}
...
, 			{
				"box" : 				{
					...
					"numinlets" : 1,
					"numoutlets" : 0,
					"patching_rect" : [ 7.0, 68.0, 620.0, 45.0 ],
					"style" : "",
					"text" : "Durant un traitement dans le domaine des harmoniques, on peut changer la directivité du champ sonore. C'est l'une des opérations sous-jacentes à l'algorithme de compensation de la distance, pour simuler le fait que le son pénètre dans la sphère ou le cercle ambisonique. (voir hoa.map~)",
					"textcolor" : [ 0.317518, 0.317518, 0.317518, 1.0 ]
				}
...
, 			{
				"box" : 				{
					"args" : [ "@obj-name", "Tutoriel 10", "@obj-desc", "Wider" ],
					"bgmode" : 0,
					"border" : 0,
					"clickthrough" : 0,
					"enablehscroll" : 0,
